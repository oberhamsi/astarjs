...
/**
   ...
   required function:
     * adjecent() -> return list of adjecent, accessible points
...
Map.prototype.adjacent = function(origin) {
  ...
  function insideMap(point) {
    return point.x >= 0 && point.x < self.heights.length &&
           point.y >= 0 && point.y < self.heights.length;
  }
  ...
  var allPoints = directions.map(function(d) { return origin.add(d); });
  return allPoints.filter(insideMap);
};
